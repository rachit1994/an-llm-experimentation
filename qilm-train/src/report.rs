...
pub struct ReportConfig<'a> {
    ...
    /// Output directory for generated per-phase reports (`PHASE-N.md`). Kept
    /// separate from `workspace_root` on purpose: provenance validation is
    /// anchored to `workspace_root` (real git repo + source tree), but the
    /// *output* location must be redirectable so tests never write into — and
    /// then delete — the committed `reports/PHASE-N.md` in the working tree.
    /// In production this is `workspace_root/reports`.
...
pub fn generate_phase_report(cfg: &ReportConfig, phase: u32) -> Result<(), ReportError> {
    ...
    let md = render_phase_markdown(&PhaseRender {
        phase,
        title: &phase_spec.title,
        gate_specs: &phase_spec.gates,
        records: &records,
        git_sha: &git_sha,
        run_ids: &run_ids,
        gates_config: &gates,
    ...
    // Write to <reports_dir>/PHASE-{phase}.md
    fs::create_dir_all(cfg.reports_dir)?;
    fs::write(cfg.reports_dir.join(format!("PHASE-{}.md", phase)), md)?;

    Ok(())
}

/// Everything `render_phase_markdown` needs, bundled so the renderer takes a
/// single argument (clippy `too_many_arguments`) and the field names document
/// the report's provenance surface at the call site.
struct PhaseRender<'a> {
    phase: u32,
    title: &'a str,
    gate_specs: &'a [PhaseGateSpec],
    records: &'a [(String, RunRecord)],
    git_sha: &'a str,
    run_ids: &'a [String],
    gates_config: &'a crate::gate::GatesConfig,
    verdict: &'a Verdict,
}

fn render_phase_markdown(v: &PhaseRender) -> String {
    let mut s = String::new();
    ...
    for gate_spec in v.gate_specs {
        let (measured, threshold, outcome) =
            evaluate_gate_for_phase(&gate_spec.name, v.gates_config, v.records);
    ...
    s.push_str("Phase ");
    s.push_str(&v.phase.to_string());
    s.push_str(" gates evaluated. ");
    s.push_str(v.verdict.as_str());
    s.push_str(".\n");
